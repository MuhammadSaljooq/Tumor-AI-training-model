...
| 8 | **Run training** by executing **`main.py` in the same Python process** (`runpy.run_path`), not as a subprocess, so **tqdm** (via `tqdm.auto`) and **prints/logs** go straight to the Colab cell output. When finished, the cell prints **`final_results.csv`** if it exists (path depends on `RESULTS_DRIVE_DIR`). |
...
| No training progress until the run finishes | Pull the latest notebook: training runs **in-process** (`runpy`) with **`tqdm.auto`** and flushed log lines. Expand the cell output if it is collapsed. |
